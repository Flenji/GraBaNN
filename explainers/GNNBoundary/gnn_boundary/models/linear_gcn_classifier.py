...
class LinearGCNClassifier(nn.Module):
    def __init__(self, node_features, num_classes, hidden_channels):
        super().__init__()
        ...
        self.lin = pyg.nn.Linear(node_features*2, hidden_channels)
    ...
    def forward(self, batch=None, embeds=None, embeds_last=None, edge_weight=None, temperature=0.05):
        if embeds_last is None:
            if embeds is None:
                ...
                #self.conv(batch.x, batch.edge_index, edge_weight=edge_weight)
                
                # 2. Readout layer
                embeds = torch.cat([
                    global_sum_pool_weighted(batch.x, batch=batch.batch, node_weight=node_weight),
                    global_mean_pool_weighted(batch.x, batch=batch.batch, node_weight=node_weight),
                ], dim=1)
